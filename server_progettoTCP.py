...
path = r'C:\\Users\\hk450\\OneDrive\Documenti\\python\\progetto\\database.txt'
    ...
    try: 
        ...
        while entry:
            ...
            #delete: 
                ...
                with db_lock:
                    conn.send("in quale tabella vuoi eliminare : dipendenti oppure zone_di_lavoro: ".encode())
                    scelta= conn.recv(1024).decode()
                    print(":::", scelta)
                    if scelta.upper() == "DIPENDENTI": 
...
def login(user, passWORD): 
    ...
    '''
    password1=h.pass_to_hash(passWORD)
    try:
        ...
        cur.execute("SELECT * FROM utenti WHERE username = %s  AND identification = %s ", (user, password1)) 
        if cur.fetchall():
            print("ci sono 2")
            return True
        else: 
...
def db_insert_zone(scelta, nome_zona, num_clienti, id_dip, rep): 
    ...
    if scelta == "zone_di_lavoro_harman_singh": 
        ...
        values= (nome_zona, num_clienti, id_dip, rep, )
        try: 
            cur.execute(insert_query, values)
            conn.commit()
            print(f"INSERIMENTO RIUSCITO")
